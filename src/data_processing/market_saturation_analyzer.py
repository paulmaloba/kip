"""
kip - Market Saturation Analyzer
...
class MarketSaturationAnalyzer:
    ...
    def analyze_by_area(self, city='Lusaka', radius_km=2):
        ...
        if 'lat' in city_businesses.columns and 'lng' in city_businesses.columns:
            city_businesses['area_cluster'] = self.cluster_by_location(
                city_businesses, radius_km
            )
            
            area_analysis = city_businesses.groupby('area_cluster').agg({
                'name': 'count',
            ...
            area_analysis.columns = ['area_id', 'business_count', 'category_distribution', 'lat', 'lng']
            
            print(f"\n🗺️  Identified {len(area_analysis)} geographic clusters")
            print(f"\n📊 Top 5 densest areas:")
            print(area_analysis.nlargest(5, 'business_count')[['area_id', 'business_count']])
    ...
    def identify_market_gaps(self, city='Lusaka', target_categories=None):
        ...
        for category in expected_categories:
            matches = sum([category.lower() in cat for cat in existing_categories])
...
def main():
    ...
    # Load data
    analyzer.load_data(
        rentech_file='data/raw/rentech_lusaka_companies.csv',
    ...
    # Analyze by area
    area_analysis = analyzer.analyze_by_area(city='Lusaka', radius_km=2)
